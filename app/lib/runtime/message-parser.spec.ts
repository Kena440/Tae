...
describe('StreamingMessageParser', () => {
  ...
  describe('no artifacts', () => {
    ...
    ])('should correctly parse chunks and strip out tae artifacts (%#)', (input, expected) => {
  ...
  describe('invalid or incomplete artifacts', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      ...
      ['Foo bar <ta', 'Foo bar '],
      ['Foo bar <tae', 'Foo bar '],
      ['Foo bar <taea', 'Foo bar <taea'],
      ['Foo bar <taeA', 'Foo bar '],
      ['Foo bar <taeArtifacs></taeArtifact>', 'Foo bar <taeArtifacs></taeArtifact>'],
      ['Before <oltArtfiact>foo</taeArtifact> After', 'Before <oltArtfiact>foo</taeArtifact> After'],
      ['Before <taeArtifactt>foo</taeArtifact> After', 'Before <taeArtifactt>foo</taeArtifact> After'],
    ])('should correctly parse chunks and strip out tae artifacts (%#)', (input, expected) => {
      runTest(input, expected);
    });
  });
  ...
  describe('valid artifacts without actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Some text before <taeArtifact title="Some title" id="artifact_1">foo bar</taeArtifact> Some more text',
        {
      ...
      [
        ['Some text before <taeArti', 'fact', ' title="Some title" id="artifact_1">foo</taeArtifact> Some more text'],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
      ...
      [
        [
          'Some text before <taeArti',
          'fac',
          't title="Some title" id="artifact_1"',
          ' ',
          '>',
          'foo</taeArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        'Before <taeArtifact title="Some title" id="artifact_1">foo</taeArtifact> After',
        {
          output: 'Before  After',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
  ...
  describe('valid artifacts with actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Before <taeArtifact title="Some title" id="artifact_1"><taeAction type="shell">npm install</taeAction></taeArtifact> After',
        {
          output: 'Before  After',
      ...
      [
        'Before <taeArtifact title="Some title" id="artifact_1"><taeAction type="shell">npm install</taeAction><taeAction type="file" filePath="index.js">some content</taeAction></taeArtifact> After',
        {
          output: 'Before  After',
